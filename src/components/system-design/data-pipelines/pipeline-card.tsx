...
import { ArrowRight, Clock, Zap, Scale, Shield, DollarSign, TrendingUp } from "lucide-react"
...
export function PipelineCard({ architecture, index }: PipelineCardProps) {
  return (
    <motion.div
    ...
    >
      <div className="relative p-6 rounded-2xl border border-border/50 bg-card hover:bg-card/80 transition-all duration-300 group-hover:border-primary/30 group-hover:shadow-xl group-hover:shadow-primary/10">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {/* Complexity Badge */}
          <span className={`px-3 py-1 text-xs font-medium rounded-full ${complexityColors[architecture.complexity]}`}>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {architecture.technologies.slice(0, 5).map((tech) => (
            <span 
              key={tech}
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium">{architecture.performance.latency}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <Zap className="w-4 h-4 text-yellow-500" />
            <span className="text-muted-foreground">Throughput:</span>
            <span className="font-medium">{architecture.performance.throughput}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <Scale className="w-4 h-4 text-green-500" />
            <span className="text-muted-foreground">Scalability:</span>
            <span className="font-medium">{architecture.performance.scalability}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <DollarSign className="w-4 h-4 text-purple-500" />
            <span className="text-muted-foreground">Cost:</span>
            <span className="font-medium">{architecture.performance.cost}</span>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="space-y-1">
            {architecture.tradeOffs.slice(0, 3).map((tradeOff) => (
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1">
            {architecture.useCases.slice(0, 3).map((useCase) => (
              <span 
                key={useCase}
        ...
        <div className="flex items-center justify-between">
          <Link href={`/system-design/data-pipelines/${architecture.id}`}>
